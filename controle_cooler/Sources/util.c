...
float AN3031_temperatura (uint16_t valor) {
	...
	temp = 25.-(valor_analog - VTEMP_25)/m;
	return temp;
...
void atualiza_velocidade(uint16_t tensao_amostra){
	TPM[1]->CONTROLS[0].CnV = TPM_CnV_VAL(tensao_amostra);
...
void atualiza_leds(float temp){
	uint32_t valor1;
	uint32_t valor2;
	valor1 = (25-temp)/(25-0)*TPM[2]->MOD;
	valor2 = (temp-25)/(50-25)*TPM[2]->MOD;
	if (temp>25){
		//TPM_desativaCh(2,0);
		TPM[2]->CONTROLS[0].CnV = valor2;
		TPM[2]->CONTROLS[1].CnV = TPM[2]->MOD +1;
		//TPM[2]->CONTROLS[0].CnV = 0;
	}
	else if(temp<25){
		//TPM_desativaCh(2,1);
		TPM[2]->CONTROLS[1].CnV = valor1;
		TPM[2]->CONTROLS[0].CnV = TPM[2]->MOD +1;
	}
	else if(temp ==25){
		TPM[2]->CONTROLS[1].CnV = 0;
		TPM[2]->CONTROLS[0].CnV = 0;
	}
}
