...
void GPIO_initLCD() {
	 ...
	 * Inicializa LCD
	 */
	delay_2us (15000);               //!< espera por mais de 30ms: 30000us/10 ->
...
void GPIO_escreveBitmapLCD (uint8_t end, uint8_t* bitmap)
{
	...
	uint8_t i;
	for (i=0; i< 8; i++) {
		GPIO_escreveByteLCD(bitmap[i], 25);
	}
}
...
void GPIO_setEndDDRAMLCD (uint8_t end)
{
	...
	GPIO_setRS (COMANDO);
	GPIO_escreveByteLCD(tmp, 25);			
}


//Ting: 1530/2 -> 765
//Veja o tipo de dado do segundo argumento de GPIO_escreveByteLCD. Eh uint8_t!
//Em 8 bits consegue-se escrever 765? Nao tem que mudar o tipo para uint16_t?
void GPIO_limpaDisplayLCD () {
	GPIO_setRS (COMANDO);
	//Ting: 
	//GPIO_escreveByteLCD(0x01, 80);			//Display Clear: 1530us/10 -> 16
	GPIO_escreveByteLCD(0x01, 765);			//Display Clear: 1530us/2 -> 765
}

//Ting: Como esta funcao depende do aplicativo ... eu nao colocaria num arquivo para reuso ...
//Vou desloca-la para main.c ...
